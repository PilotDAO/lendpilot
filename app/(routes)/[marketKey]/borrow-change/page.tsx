...
export default async function BorrowChangePage({
  params,
  searchParams,
}: {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6 text-gray-900 dark:text-white">
        Borrow Changes - {marketKey}
      </h1>
      <p className="text-gray-600 dark:text-gray-400 mb-6">
        View borrowing trends and changes over time
      </p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">
          Total Borrowing Over Time
        </h2>
        <TrendsTotalChart data={data.data} mode="borrow" />
      </div>

      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">
          Borrowing by Asset (Top 10)
        </h2>
        <TrendsByAssetChart assets={data.assetChanges} mode="borrow" limit={10} />
      </div>

      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
        <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">
          Borrow Changes by Asset
        </h2>
        <TrendsChangesTable data={data.assetChanges} mode="borrow" />
      </div>
    </div>
  );
}
